...
if [ -z $1 ] 
     then 
       ...
       echo 
       echo "          Add 'v' as second parameter for just ext, no sound"
       echo
       echo "Edit script to turn on/off condition reporting and C or F temperature"
       echo
       exit 0 
fi 
...
#        wunder_code=${1:2:10}
        w_type="wunder"
        current=`curl --connect-timeout 15 -s  http://api.wunderground.com/weatherstation/WXCurrentObXML.asp?ID=$wunder_code | perl -ne '{chomp;/\<temp_f\>(.*)?\<\/temp_f\>/; print "\n$1"; }'`
